...
{{--        <span>{{ \App\CPU\translate('amount') }}:{{ $order->collected_cash ? $order->collected_cash . ' ' . \App\CPU\Helpers::currency_symbol() : 0 . ' ' . \App\CPU\Helpers::currency_symbol() }}</span>--}}
    @if ($order->payment_id == 1)
            <span>{{ \App\CPU\translate('Paid Amount') }}:{{ $order->collected_cash ? $order->collected_cash . ' ' . \App\CPU\Helpers::currency_symbol() : 0 . ' ' . \App\CPU\Helpers::currency_symbol() }}</span>

            <span>{{ \App\CPU\translate('Due Cash') }}:{{ number_format($order->collected_cash - $order->order_amount - $order->total_tax + $order->extra_discount + $order->coupon_discount_amount, 2) }}
